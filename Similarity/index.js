...
function displayMatch(a, b, matrix) {
    ...
    function step(ind, val) {
        let el = document.createElement("span");
        if (ind != null)
            el.innerText = toText(val);
        return el;
    }
    for (let i = 0; i < aVal.length; i++) {
        ...
        if (aI == null && bI != null)
            bEl.classList.add("sub");
        if (bI == null && aI != null)
            aEl.classList.add("sub");
...
function calcNeedleman(a, b) {
    ...
    for (let y = 1; y <= a.length; y++) {
        ...
        for (let x = 1; x < width; x++) {
            let bVal = b[x];
            matrix[x + yOff] = Math.max(matrix[(x - 1) + (yOff - width)] + (aVal == bVal ? match : mismatch), matrix[(x - 1) + yOff] + gap, matrix[x + (yOff - width)] + gap);
        }
    }
    return matrix;
}
// https://en.wikipedia.org/wiki/Smith-Waterman_algorithm
function calcWaterman(a, b) {
    const width = b.length + 1;
    let matrix = new Int32Array((a.length + 1) * (b.length + 1));
    const match = 1;
    const mismatch = -1;
    ...
    for (let y = 1; y <= a.length; y++) {
        ...
        for (let x = 1; x < width; x++) {
            let bVal = b[x];
            matrix[x + yOff] = Math.max(matrix[(x - 1) + (yOff - width)] + (aVal == bVal ? match : mismatch), matrix[(x - 1) + yOff] + gap, matrix[x + (yOff - width)] + gap, 0);
        }
    }
    return matrix;
}
