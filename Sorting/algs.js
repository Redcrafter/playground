...
async function DoubleSelectionSort(arr) {
    let left = 0, right = arr.length - 1, smallest = 0, biggest = 0;
    while (left <= right) {
...
async function BinarySearchTreeSort(arr) {
...
async function CircleSort(arr) {
    async function circleSort(lo, hi, swaps) {
        if (lo === hi) {
            return swaps;
        }
        const high = hi;
...
async function RadixLSD(arr) {
    const radix = 4;
    let wr = arr.length / radix;
    function digit(a, power) {
