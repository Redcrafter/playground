...
let displayMethod: DrawMode = DrawMode.Ramp;
...
let running = false;
async function Run(sortFunc: (a: SortArray) => Promise<void>) {
    if (running) {
        return;
    }
    running = true;
...
function Draw() {
    ...
    function setColor(index: number, val: number) {
