...
async function insertPoint(p) {
    ...
    for (const triangle of badTriangles) {
        ...
        if (!e1)
            poly.push([triangle[0], triangle[1]]);
        if (!e2)
            poly.push([triangle[1], triangle[2]]);
        if (!e3)
            poly.push([triangle[2], triangle[0]]);
    }
    for (const edge of poly) {
        let t = [edge[0], edge[1], p];
        while (!ccw(t)) {
            shuffle(t);
        }
...
function circumcircle(a, b, c) {
    let A = b.x - a.x, B = b.y - a.y, C = c.x - a.x, D = c.y - a.y, E = A * (a.x + b.x) + B * (a.y + b.y), F = C * (a.x + c.x) + D * (a.y + c.y), G = 2 * (A * (c.y - b.y) - B * (c.x - b.x)), minx, miny, dx, dy;
